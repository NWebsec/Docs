...
`upgrade-insecure-requests` is the latest addition to the CSP directives, but note that it lives in a [separate specification](https://www.w3.org/TR/upgrade-insecure-requests/). Though it looks very innocent and straight forward to configure, there are some important considerations before employing it.
...
[NWebsec.AspNetCore.Middleware](NWebsec.AspNetCore.Middleware.html) lets you enable `upgrade-insecure-requests` through your OWIN startup class:
