...
The NWebsec.SessionSecurity library offers protection against session fixation attacks on authenticated users in ASP.NET applications. Using the [AuthenticatedSessionIDManager](AuthenticatedSessionIDManager.html), session IDs are tightly coupled with the identity of the logged in user. The result: Session IDs cannot be reused across users.

## How it works

The [AuthenticatedSessionIDManager](AuthenticatedSessionIDManager.html) will check the following when creating/validating a session ID:
