...
title: "NWebsec.Mvc"
nav_order: 3
parent: "NWebsec libraries"
---

# NWebsec.Mvc (ASP.NET 4)

NWebsec.Mvc lets you configure NWebsec through MVC ActionFilter attributes. MVC 3 or newer is supported. By decorating controllers and actions with attributes, the [NWebsec](NWebsec.html) configuration is overridden. This is how it works:

NWebsec.Mvc depends on the NWebsec package, which includes the HttpHeaderModule. This module relies on its [Configuration](Configuration.html), if there is any. Now, the attributes in NWebsec.Mvc lets you override the HttpHeaderModule's configuration, here's the order in which the final configuration is calculated:
...
Most elements from the [Configuration](Configuration.html) have an MVC attribute counterpart.
