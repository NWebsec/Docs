...
You'll notice that not all features from the [NWebsec](NWebsec.html) library are available yet, e.g. cache-headers, these are under consideration.
...
The middleware is documented alongside the web.config and MVC attributes. Refer to the [Configuration](Configuration.html) for samples.
