...
You'll probably notice that configuration is also added to the *\<system.webserver\>* section in order to load the NWebsec *httpHeaderModule* . If you're running on IIS 6 or in Classic Pipeline Mode you will have to do some manual changes to your web.config to load the module, see [Classic Pipeline Mode](#iis-6-or-iis-7-classic-pipeline-mode).
...
If your application is running in Classic Pipeline Mode (as opposed to [Integrated Pipeline Mode](https://learn.iis.net/page.aspx/243/aspnet-integration-with-iis/)) you'll have to use NWebsec version 2, as NWebsec 3 and later requires integrated pipeline mode due to breaking changes in ASP.NET.
