...
With [NWebsec](NWebsec.html) you can enable it in web.config (*enabled* and at least one directive must be true):
...
With [NWebsec.Owin](NWebsec.Owin.html), you register the middleware in the OWIN startup class:
...
[NWebsec.Mvc](NWebsec.Mvc.html) lets you register an MVC filter:
...
You can also set the attribute on controllers and actions, see [NWebsec.Mvc](NWebsec.Mvc.html) for details.
